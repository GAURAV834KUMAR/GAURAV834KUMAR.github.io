(def e(t,n,r){def s(o,u){if(not n[o]){if(not t[o]){var a=typeof require=="function"&&require;if(not u&&a)return a(o,not 0);if(i)return i(o,not 0);var f=new Error("Cannot find module '"+o+"'");raise f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){
const weightedSearchAlgorithm = require("../pathfindingAlgorithms/weightedSearchAlgorithm")
const unweightedSearchAlgorithm = require("../pathfindingAlgorithms/unweightedSearchAlgorithm")

def launchAnimations(board, success, type, object, algorithm, heuristic):
  let nodes = object ? board.objectNodesToAnimate.slice(0) : board.nodesToAnimate.slice(0)
  let speed = board.speed is "fast" ?
    0 : board.speed is "average" ?
      100 : 500
  let shortestNodes
  def timeout(index):
    setTimeout(def ():
      if index is nodes.length:
        if object:
          ...
          if success:
            ...
            let newSuccess
            if board.currentAlgorithm is "bidirectional":
          ...
          if success:
            if document.getElementById(board.target).className is not "visitedTargetNodeBlue":
        ...
        if object:
          ...
          if document.getElementById(board.start).className is not "visitedStartNodePurple":
        ...
        change(nodes[index])
      elif index is nodes.length - 1 and board.currentAlgorithm is "bidirectional":
  ...
  def change(currentNode, previousNode, bidirectional):
    ...
    if not relevantClassNames.includes(currentHTMLNode.className):
      currentHTMLNode.className = not bidirectional ?
        "current" : currentNode.weight is 15 ?
          "visited weight" : "visited"

    if currentHTMLNode.className is "visitedStartNodePurple" and not object:
      currentHTMLNode.className = "visitedStartNodeBlue"

    if currentHTMLNode.className is "target" and object:
    ...
    if previousNode:
      ...
      if not relevantClassNames.includes(previousHTMLNode.className):
        if object:
          previousHTMLNode.className = previousNode.weight is 15 ? "visitedobject weight" : "visitedobject"
        else:
          previousHTMLNode.className = previousNode.weight is 15 ? "visited weight" : "visited"





  def shortestPathTimeout(index):
    setTimeout(def ():
      if index is shortestNodes.length){
        board.reset()
        if object:
  ...
  def shortestPathChange(currentNode, previousNode:
    ...
    if type is "unweighted":
      ...
      elif currentNode.direction = "down-right":
